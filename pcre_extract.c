...
static
void regexp_extract(sqlite3_context *ctx, int argc, sqlite3_value **argv)
{
    const char *re, *str;
    pcre *p;
    pcre_extra *e;
	...
	// return;
	int offsets[6];
	rc = pcre_exec(p, e, str, strlen(str), 0, 0, offsets, 6);
    // char* offsets_string;
    // offsets_string = (char*)calloc(100, sizeof(char));
    // sprintf(offsets_string, "%d-%d-%d-%d-%d-%d", offsets[0], offsets[1], offsets[2], offsets[3], offsets[4], offsets[5]);
    // // sqlite3_result_int(ctx, rc);
    // sqlite3_result_text(ctx, offsets_string, -1, free);
    // return;
	...
	int match_size = offsets[3] - offset;
	char* res;
...
int sqlite3_extension_init(sqlite3 *db, char **err, const sqlite3_api_routines *api)
{
	...
	sqlite3_create_function(db, "REGEXP_EXTRACT", 2, SQLITE_UTF8, cache, regexp_extract, NULL, NULL);
	return 0;
}
